...
test("a retype that would orphan a CHILD is refused", () => {
  const { root, projects } = board();
  put(projects, "OBA-1", "goal");
  put(projects, "OBA-2", "requirement", { parent: "OBA-1" });
  ...
  const res = applyEdit(projects, "OBA-3", { type: "architecture" });
...
test("a legal retype with children succeeds and leaves every edge legal", () => {
  const { root, projects } = board();
  put(projects, "OBA-1", "goal");
  put(projects, "OBA-2", "requirement", { parent: "OBA-1" });
...
test("a retype to an unknown type is refused", () => {
  const { root, projects } = board();
  put(projects, "OBA-1", "goal");
  put(projects, "OBA-2", "feature", { parent: "OBA-1" });
  const res = applyEdit(projects, "OBA-2", { type: "nonsense" });
  assert.equal(res.ok, false);
  assert.equal(typeOf(projects, "OBA-2"), "feature");
  rmSync(root, { recursive: true, force: true });
});

test("id and project stay read-only", () => {
  const { root, projects } = board();
  put(projects, "OBA-1", "goal");
  for (const f of ["id", "project"]) {
    const res = applyEdit(projects, "OBA-1", { [f]: "NOPE-9" });
