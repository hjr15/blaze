...
test("applyEdit patches an allowed field and writes in place", async () => {
  const { root, projects } = fixture();
  const r = await applyEdit(projects, "OBA-1", { assignee: "ryan", priority: "high" }, { today: "2026-07-01" });
...
test("applyEdit rounds estimate to 5m", async () => {
  const { root, projects } = fixture();
  const r = await applyEdit(projects, "OBA-1", { estimate: 47 }, {});
  assert.equal(r.ok, true);
  assert.match(readFileSync(r.file, "utf8"), /estimate: 45/);
  rmSync(root, { recursive: true, force: true });
});

test("applyEdit rejects an unknown patch field with no write", async () => {
  const { root, projects } = fixture();
  const before = readFileSync(join(projects, "OBA", "defined", "OBA-1.md"), "utf8");
  const r = await applyEdit(projects, "OBA-1", { status: "done" }, {});
...
test("applyEdit rejects an invalid enum value with no write", async () => {
  const { root, projects } = fixture();
  const before = readFileSync(join(projects, "OBA", "defined", "OBA-1.md"), "utf8");
  const r = await applyEdit(projects, "OBA-1", { priority: "banana" }, {});
  assert.equal(r.ok, false);
  assert.equal(readFileSync(join(projects, "OBA", "defined", "OBA-1.md"), "utf8"), before);
  rmSync(root, { recursive: true, force: true });
});

test("applyEdit rejects a parent that violates the hierarchy, no write", async () => {
  const { root, projects } = fixture();
  // A task's parent must be an epic; point it at another task → invalid.
  mkdirSync(join(projects, "OBA", "defined"), { recursive: true });
  writeFileSync(join(projects, "OBA", "defined", "OBA-2.md"),
    "---\nid: OBA-2\ntitle: t2\ntype: task\nproject: OBA\nestimate: 30\n---\nb\n");
  const r = await applyEdit(projects, "OBA-1", { parent: "OBA-2" }, {});
  assert.equal(r.ok, false);
  assert.ok(r.errors.some((e) => /invalid parent/.test(e)));
  rmSync(root, { recursive: true, force: true });
});

test("applyEdit accepts a title change", async () => {
  const { projects } = fixture();
  const r = await applyEdit(projects, "OBA-1", { title: "renamed" }, {});
  assert.equal(r.ok, true);
});

test("applyEdit hard-rejects an off-taxonomy label", async () => {
  const { root, projects } = fixture();
  writeFileSync(join(projects, "OBA", "project.json"), JSON.stringify({ components: [], labels: ["area:cms"] }));
  const r = await applyEdit(projects, "OBA-1", { labels: "area:cms, bogus" }, { today: "2026-07-15" });
  assert.equal(r.ok, false);
  assert.ok(r.errors.some((e) => /bogus/.test(e)));
  rmSync(root, { recursive: true, force: true });
});

test("applyEdit throws on malformed project.json instead of silently skipping taxonomy", async () => {
  const { root, projects } = fixture();
  writeFileSync(join(projects, "OBA", "project.json"), "{ not valid json");
  await assert.rejects(
    async () => applyEdit(projects, "OBA-1", { labels: "bogus" }, { today: "2026-07-15" }),
    /cannot parse/,
  );
  rmSync(root, { recursive: true, force: true });
});

test("applyEdit accepts a registered sprint id", async () => {
  const { root, projects } = fixture();
  writeFileSync(join(root, "sprints.json"), JSON.stringify({
    active: "S1", sprints: [{ id: "S1", name: "Mid-July", start: "2026-07-13", end: "2026-07-26" }],
  }));
  const r = await applyEdit(projects, "OBA-1", { sprint: "S1" }, {});
  assert.equal(r.ok, true, JSON.stringify(r.errors));
  assert.match(readFileSync(r.file, "utf8"), /sprint: S1/);
  rmSync(root, { recursive: true, force: true });
});

test("applyEdit rejects an unregistered sprint id, no write", async () => {
  const { root, projects } = fixture();
  const before = readFileSync(join(projects, "OBA", "defined", "OBA-1.md"), "utf8");
  const r = await applyEdit(projects, "OBA-1", { sprint: "S9" }, {});
  assert.equal(r.ok, false);
...
test("applyEdit skips taxonomy (no crash) for a ticket with no project field", async () => {
  ...
  const r = await applyEdit(projects, "OBA-1", { labels: "anything" }, { today: "2026-07-15" });
  assert.equal(r.ok, true, JSON.stringify(r.errors));
  rmSync(root, { recursive: true, force: true });
});
