...
// BLZ-474 / ADR-0026: the `docs/` assertion below is LOAD-BEARING, not incidental tidiness.
...
// therefore reverses ADR-0026; do it deliberately, with `tests/shipped-doc-links.test.mjs`
...
test("npm pack ships engine only — no tests, no data dirs, no dotfiles beyond defaults", () => {
  ...
  for (const f of files) {
    ...
    assert.ok(!f.startsWith("docs/"), `docs must not ship (ADR-0026): ${f}`);
