...
export const clientScript = `
    ...
    async function pollLive(){
      ...
      try{
        const {groups,unreadable}=await (await fetch("/api/live")).json();
        // BLZ-493 / ADR-0031: a feed the server COULD NOT READ must not render as a feed it
        // read and found empty. This branch is first for that reason — falling through to
        // "No recent activity." would be the silent drop the whole guard exists to stop.
        if(unreadable){ el.innerHTML='<div class="empty">ACTIVITY FEED UNREADABLE — this is not "no activity": '+esc(unreadable.path)+' could not be read, so nothing below reflects what agents are doing.</div>'; return; }
