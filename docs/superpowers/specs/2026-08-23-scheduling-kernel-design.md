...
| the five `ticket` columns (§2) | `ticket` | `ALTER TABLE ADD COLUMN`, the **9.0 ms metadata-only path** (ADR-0018), never a generated column; `STRICT` stays on every SQLite table that holds them. **This sentence was FALSE when written and is TRUE now — the sequence matters more than the current state.** BLZ-376 measured it: zero of the seven tables in `SQLITE_DDL` carried `STRICT`, the only occurrence of the word in `sqlite-schema.mjs` being `ON DELETE RESTRICT`, so the spec had generalised from the v4 modules (`link`, `hierarchy`, `view`, which take ` STRICT` from `sql-dialect.mjs`'s `tbl` token) to the whole schema. BLZ-390 then measured the objection to fixing it — **0 of 21,372 live rows** violate a `STRICT` declaration — and made all seven `STRICT`, taking the shadow to **DB schema version 3**. The `projection-schema.mjs` tables are deliberately still not `STRICT`: `projection_meta.config_version` is declared `bigint`, which is not a `STRICT`-legal type. |
