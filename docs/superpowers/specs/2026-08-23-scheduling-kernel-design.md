...
| the five `ticket` columns (§2) | `ticket` | `ALTER TABLE ADD COLUMN`, the **9.0 ms metadata-only path** (ADR-0018), never a generated column; `STRICT` stays on every SQLite table that holds them. **This sentence was FALSE when written and is TRUE now — the sequence matters more than the current state.** BLZ-376 measured it: zero of the seven tables in `SQLITE_DDL` carried `STRICT`, the only occurrence of the word in `sqlite-schema.mjs` being `ON DELETE RESTRICT`, so the spec had generalised from the v4 modules (`link`, `hierarchy`, `view`, which take ` STRICT` from `sql-dialect.mjs`'s `tbl` token) to the whole schema. BLZ-390 then measured the objection to fixing it and it did not survive, and the figure is stated with its population because an earlier draft was not: **0 rows violate a `STRICT` declaration** — **19,686** in the seven tables STRICT was applied to, and **21,413** across all **26** installed tables, of which **1,724** sit in tables STRICT was *never* applied to. (25/21,411 in an earlier draft excluded `sqlite_sequence`, which SQLite creates itself, without saying so.) All seven are `STRICT` now, plus `blaze_meta`, taking the shadow to **DB schema version 3**. `ticket_event` contributes **0** rows: `loadCorpus` writes no events, so the audit-trail table has no measured evidence behind it either way. The `projection-schema.mjs` tables are deliberately still not `STRICT`: `projection_meta.config_version` is declared `bigint`, which is not a `STRICT`-legal type. |
