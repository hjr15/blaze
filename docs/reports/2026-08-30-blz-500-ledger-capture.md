...
parseable) — so it is latent, not live.

**It is NOT fixed on this branch, and this report must not be read as saying it is.** Closing it
needs the parse failure propagated out of `parseLines` to every caller, a decision about what the
drain does with bytes it cannot parse, and a new on-disk artifact to hold them. That is a subsystem
with its own contract, not a clause of this ticket, and it is going through its own ticket, PR and
review cycle. Until that lands, the honest statement of this section's question is:

- **The CronJob has never dropped an op** — established by the code path above, not by the count.
- **`blaze commit` has a path that would destroy an unparseable ledger line**, verified by
  construction, never yet triggered on this board, and still open.
