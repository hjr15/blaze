...
the blast radius of the refusal changed. Exit codes: **0** every queue was read in
full, **2** the report is incomplete, **1** the verb refused to run at all (unknown
flag, `BLAZE_READONLY`, no session identity, lock held, foreign branch).

**A queue read only in PART is incomplete too** — the fourth shape, and the only
silent one. A ledger line that will not parse (classically a partial final line from
a process killed mid-append, which is exactly the abandoned-session condition) is
skipped rather than thrown on, so a good ledger still drains. But the skip used to
go no further than a warning on stderr: the queue came back short and was rendered
as fully read, its partial buckets summed into the totals, exit 0 — and a caller
reading the exit code never saw the warning. Such a queue is now marked `PARTIALLY
READ — N line(s) could not be parsed`, excluded from the totals with its own
disclaimer, and exits **2**. It is not the same as `state UNKNOWN`: an unreadable
queue yields nothing, a partial one yields every op that *did* parse, and those ops
are real work that must still be reported.

**A flush quarantines what it could not parse, rather than erasing it.** The drain
reads the whole file to know how many bytes to clear, so clearing used to destroy an
unparseable line along with the ops that were successfully committed — the one path
on which `blaze commit` could lose a record for good. The flush still proceeds; the
raw lines are appended to `.blaze/pending/<session>.corrupt` first and the run says
so on stderr. The sidecar deliberately does not end in `.jsonl`, so it is never
picked up as a queue — which would make the condition self-perpetuating, its
contents being unparseable by definition.
